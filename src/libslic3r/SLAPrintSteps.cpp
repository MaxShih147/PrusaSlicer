...
void SLAPrint::Steps::apply_printer_corrections(SLAPrintObject &po, SliceOrigin o)
{
    ...
    const int blc = m_print->m_bottom_layer_count;
    ...
    // Normal layers (index >= bottom_layer_count)
    {
        ...
        if (m_print->m_tolerance_compensation && (ca != 0 || cb != 0)) {
            BOOST_LOG_TRIVIAL(info) << "[tolerance] applying tc a=" << m_print->m_tolerance_compensation_a
                                    << " b=" << m_print->m_tolerance_compensation_b
                                    << " layers=" << blc << ".." << (int)po.m_slice_index.size();
    ...
    // Bottom layers (index < bottom_layer_count)
    {
        ...
        if (m_print->m_bottom_tolerance_compensation && (ca != 0 || cb != 0)) {
            ...
            BOOST_LOG_TRIVIAL(info) << "[tolerance] applying btc a=" << m_print->m_bottom_tolerance_compensation_a
                                    << " b=" << m_print->m_bottom_tolerance_compensation_b
                                    << " layers=0.." << (int)n;
